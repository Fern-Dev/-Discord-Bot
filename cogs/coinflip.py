            ...
            if id == str(messageID) and v['type'] == 'coinflip':
              ...
              if str(v['creator']) != str(payload.user_id):
                ...
                if ticket_number <= 0.5:
                  ...
                  main.users[str(user_original.id)]['balance'] = int(main.users[str(user_original.id)]['balance']) + int(v['bet'])
                  main.users[str(user_opponent.id)]['balance'] = int(main.users[str(user_opponent.id)]['balance']) - int(v['bet'])
                  main.saveDB('users', main.users)
